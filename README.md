...
项目背景：
比如我们有一张接口表，里面维护了所有的接口路径、接口作用等基本信息，这张表基本不会变动。然后我的首页菜单需要关联这张表、内部的鉴权需要依赖这张表等等。当我更新这张表的时候，菜单数据和鉴权这两部分的缓存都需要进行删除，这样业务再发起请求的时候，才会拉取到数据库中最新的数据。但是我在更新这张表的时候，我可能不知道我被鉴权这个场景依赖到了，所以就导致我没有对这部分的缓存进行更新。
...
### 1. 添加依赖（需要自行下载源码通过 maven 进行 package）
